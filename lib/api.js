...
api.buildOne = function (data) {
  ...
  API_NAME = util.filterForName(API_NAME);
  const API_dESCRIBE = data.name || "";
  let API_URL = `"${data.path.split("?")[0]}"`; //过滤path参数后缀
  ...
  const API_ANNOTATION =
    api_annotation_head + api_annotation_body + api_annotation_foot;
  ...
  if (fs.existsSync(targetApiFilePath)) {
    ...
    // 检查目标文件内是否已有该接口
    let matchText = "";
    matchText = `export async function ${API_NAME}(`;
