...
util.getApiData = async function () {
    const { TYPE = 2, API_FILE_PATH = path.resolve(process.cwd(), './api.json'), NEI_SERVER = 'https://nei.netease.com', NEI_PID, PRIVATE_TOKEN } = apiConfig;
    let apiJson = {}
    if (TYPE === 1) {
...
util.isExists = function (showErr) {
    ...
    if (fs.existsSync(path.resolve(process.cwd(), './nei.config.js'))) {
        ...
        showErr && log.error("请确认当前目录下是否已配置<nei.config.js>文件\n详见文档：https://github.com/AllenPan03/nei-api")
    }
    return result
}
module.exports = util;